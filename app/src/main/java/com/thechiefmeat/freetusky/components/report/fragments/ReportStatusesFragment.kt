...
package com.thechiefmeat.freetusky.components.report.fragments
...
import com.thechiefmeat.freetusky.AccountActivity
import com.thechiefmeat.freetusky.R
import com.thechiefmeat.freetusky.ViewMediaActivity
import com.thechiefmeat.freetusky.ViewTagActivity
import com.thechiefmeat.freetusky.components.report.ReportViewModel
import com.thechiefmeat.freetusky.components.report.Screen
import com.thechiefmeat.freetusky.components.report.adapter.AdapterHandler
import com.thechiefmeat.freetusky.components.report.adapter.StatusesAdapter
import com.thechiefmeat.freetusky.db.AccountManager
import com.thechiefmeat.freetusky.di.Injectable
import com.thechiefmeat.freetusky.di.ViewModelFactory
...
import com.thechiefmeat.freetusky.util.hide
import com.thechiefmeat.freetusky.util.show
import com.thechiefmeat.freetusky.viewdata.AttachmentViewData
...
class ReportStatusesFragment : Fragment(), Injectable, AdapterHandler {
    ...
    private fun initStatusesView() {
        ...
        viewModel.networkStateAfter.observe(viewLifecycleOwner, Observer {
            if (it?.status == com.thechiefmeat.freetusky.util.Status.RUNNING)
                progressBarBottom.show()
            else
                progressBarBottom.hide()

            if (it?.status == com.thechiefmeat.freetusky.util.Status.FAILED)
                showError(it.msg)
        })
        ...
        viewModel.networkStateRefresh.observe(viewLifecycleOwner, Observer {
            if (it?.status == com.thechiefmeat.freetusky.util.Status.RUNNING && !swipeRefreshLayout.isRefreshing)
